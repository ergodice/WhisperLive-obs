...
class WhisperToOBSBridge:
    ...
    def is_final_text(self, segments):
        """
        セグメント情報から確定済みテキストかを判定
        
        Args:
            segments: セグメント情報のリスト
            
        Returns:
            bool: すべてのセグメントが確定済みの場合 True
        """
        if not segments:
            return False
        
        # すべてのセグメントが is_final=True の場合、確定済みと判定
        return all(segment.get('is_final', False) for segment in segments)
    ...
    def transcription_callback(self, text, segments):
        ...
        if self.final_text_only:
            if self.is_final_text(segments):
                print("[FINAL]: 確定済みテキストを OBS に送信")
                self.update_text_source(text)
            else:
                print("[INTERIM]: 確定前のテキスト（スキップ）")
        else:
            # すべてのテキストを表示
            self.update_text_source(text)
